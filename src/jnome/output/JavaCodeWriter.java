...
public class JavaCodeWriter extends Syntax {
	...
	// }
	//
	// public String toCodeActualParameter(ActualArgument parameter) throws
	// LookupException {
	...
	public String toCodeNamespaceOrTypeReference(
			NamespaceOrTypeReference typeReference) throws LookupException {
	...
	public String toCodeIntersectionTypeReference(
			JavaIntersectionTypeReference typeReference) throws LookupException {
		StringBuffer result = new StringBuffer();
		Iterator<? extends TypeReference> iter = typeReference.typeReferences()
				.iterator();
		while (iter.hasNext()) {
		...
		// return
		// toCode(typeReference.leftHandSide())+" & "+toCode(typeReference.rightHandSide());
	...
	public String toCodeBasicTypeReference(BasicJavaTypeReference typeReference)
			throws LookupException {
		StringBuffer result = new StringBuffer(toCode(typeReference.getTarget()));
		if (result.length() > 0) {
			result.append(".");
		}
		result.append(typeReference.signature());
		List<ActualTypeArgument> typeArguments = typeReference.typeArguments();
		
		appendTypeParameters(typeArguments, result);
	...
	public String toCodeNamespacePart(NamespacePart part)
			throws LookupException {
		StringBuffer result = new StringBuffer();
	...
	public String toCodeCompilationUnit(CompilationUnit cu)
			throws LookupException {
		StringBuffer result = new StringBuffer();
	...
	public boolean isClass(Element element) {
		return (element instanceof RegularType)
				&& ((Type) element).is(((Java) language()).INTERFACE) != Ternary.TRUE;
	}
	...
	public String toCodeClassBlock(Type type) throws LookupException {
		try {
			...
			List<? extends TypeElement> members = type
					.directlyDeclaredElements();
			// Members
			new RobustVisitor() {
	...
	public boolean isAnonymousClass(Element element) {
		return (element instanceof Type)
				&& (element.parent() instanceof ConstructorInvocation);
	}
	...
	public String toCodeClass(Type type) throws LookupException {
		try {
			...
			List<SubtypeRelation> superTypes = type
					.nonMemberInheritanceRelations(SubtypeRelation.class);
			...
			for (SubtypeRelation relation : superTypes) {
				...
				if (relation
						.is(relation.language(Java.class).IMPLEMENTS_RELATION) == Ternary.TRUE) {
	...
	private <T extends Element> void appendTypeParameters(List<T> parameters,
			final StringBuffer result) throws LookupException {
		if (!parameters.isEmpty()) {
			result.append("<");
			Iterator<T> iter = parameters.iterator();
			while (iter.hasNext()) {
				result.append(toCode(iter.next()));
				if (iter.hasNext()) {
					result.append(",");
		...
		try {
			...
			new AbstractPredicate<SubtypeRelation>() {
				public boolean eval(SubtypeRelation rel) throws LookupException {
					return !toCode((rel).superClassReference()).equals(
							"java.lang.Object");
				}
			}.filter(superTypes);
			if (superTypes.size() > 0) {
				...
				while (iter.hasNext()) {
					TypeReference tr = ((AbstractInheritanceRelation) iter
							.next()).superClassReference();
	...
	public String toCodeMethod(Method<?, ?, ?, ?> method)
		...
		result.append(((SimpleNameDeclarationWithParametersSignature) method
				.signature()).name());
		result.append("(");
	...
	public String toCodeExceptionClause(ExceptionClause ec)
		...
		while (iter.hasNext()) {
			result.append(toCodeExceptionDeclaration((ExceptionDeclaration) iter
					.next()));
			if (iter.hasNext()) {
				result.append(", ");
			}
	...
	// public String toCodeMemberVariable(RegularMemberVariable var) throws
	// LookupException {
	...
	public String toCodeCatchClause(CatchClause cc) throws LookupException {
		return "catch (" + toCodeVariable(cc.getExceptionParameter()) + ") "
				+ toCode(cc.statement());
	}
	...
	public String toCodeSynchronized(SynchronizedStatement ts)
			throws LookupException {
		return "synchronized(" + toCode(ts.expression()) + ") "
				+ toCode(ts.getStatement());
	}
	...
	public String toCodeStatementExprList(StatementExprList sel)
		...
		while (iter.hasNext()) {
			result.append(toCode(((StatementExpression) iter.next())
					.getExpression()));
			if (iter.hasNext()) {
				result.append(", ");
			}
	...
	public String toCodeVariableDeclarator(VariableDeclarator local)
		...
		try {
			new RobustVisitor<VariableDeclaration<LocalVariable>>() {
				...
				public Object visit(VariableDeclaration<LocalVariable> element)
						throws LookupException {
				...
				public void unvisit(VariableDeclaration<LocalVariable> el,
						Object undo) {
					// NOP
				}
	...
	public String toCodeFor(ForStatement statement) throws LookupException {
		return "for " + toCode(statement.forControl())
				+ toCode(statement.getStatement());
	}
	...
	public String toCodeSimpleForControl(SimpleForControl control)
			throws LookupException {
		return "(" + toCodeForInit((Element) control.getForInit()) + "; "
				+ toCode(control.condition()) + "; " + toCode(control.update())
				+ ") ";
	}
	...
	public String toCodeEnhancedForControl(EnhancedForControl control)
			throws LookupException {
		return "(" + toCodeForInit((Element) control.variableDeclarator())
				+ ": " + toCode(control.collection()) + ") ";
	}
	...
	public String toCodeDo(DoStatement statement) throws LookupException {
		return "do " + toCode(statement.getStatement()) + "while("
				+ toCode(statement.condition()) + ");";
	}
	...
	public String toCodeInfixInvocation(InfixOperatorInvocation inv)
		...
		return "(" + toCode(inv.getTarget()) + ") " + inv.name() + " ("
				+ toCode((Element) inv.getActualParameters().get(0)) + ")";
	}
	...
	public String toCodeThisConstructorDelegation(
			ThisConstructorDelegation deleg) throws LookupException {
	...
	public String toCodeSuperConstructorDelegation(
			SuperConstructorDelegation deleg) throws LookupException {
	...
	public String toCodeInstanceOf(InstanceofExpression ioe)
			throws LookupException {
		return "(" + toCode(ioe.getExpression()) + " instanceof "
				+ toCode(ioe.getTypeReference()) + ")";
	}
	...
	public String toCodeDimInit(DimensionInitializer init)
			throws LookupException {
		return "["
				+ (init.getExpression() != null ? toCode(init.getExpression())
						: "") + "]";
	}
	...
	public String toCodeCondExpr(ConditionalExpression ce)
			throws LookupException {
		return "(" + toCode(ce.getCondition()) + " ? " + toCode(ce.getFirst())
				+ " : " + toCode(ce.getSecond()) + ")";
	}
	...
	public String toCodeCast(ClassCastExpression cc) throws LookupException {
		return "((" + toCode(cc.getTypeReference()) + ")"
				+ toCode(cc.getExpression()) + ")";
	}
	...
	public String toCodeAssignment(AssignmentExpression expr)
			throws LookupException {
		return toCode((Element) expr.getVariable()) + " = "
				+ toCode(expr.getValue());
	}
	...
	public static void writeCode(Arguments arguments) throws IOException,
		...
		for (Type type : types) {
			...
			File out = new File(arguments.getOutputDirName()
					+ File.separatorChar + relDirName + File.separatorChar
					+ fileName);
